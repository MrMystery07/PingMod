...
public abstract class EntityPlayer extends EntityLivingBase {
	...
	public void onLivingUpdate() {
		...
		List<EnumParticleTypes> particleList = new ArrayList<EnumParticleTypes>();

		particleList.add(EnumParticleTypes.PORTAL);
		particleList.add(EnumParticleTypes.ENCHANTMENT_TABLE);
		particleList.add(EnumParticleTypes.FLAME);
		particleList.add(EnumParticleTypes.CRIT_MAGIC);
		particleList.add(EnumParticleTypes.LAVA);
		particleList.add(EnumParticleTypes.EXPLOSION_NORMAL);
		particleList.add(EnumParticleTypes.CRIT);
		particleList.add(EnumParticleTypes.SNOW_SHOVEL);
		particleList.add(EnumParticleTypes.SPELL_WITCH);
		if (Controller.particle) {
			this.worldObj.spawnParticle(particleList.get(Controller.particleCount),
					this.posX + (this.rand.nextDouble() - 0.5D) * (double) this.width,
					this.posY + this.rand.nextDouble() * (double) this.height - 0.2,
					this.posZ + (this.rand.nextDouble() - 0.5D) * (double) this.width,
					(this.rand.nextDouble() - 0.5D) * 2.0D, -this.rand.nextDouble(),
					(this.rand.nextDouble() - 0.5D) * 2.0D, 1);
		}
