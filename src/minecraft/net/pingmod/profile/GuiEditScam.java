...
public class GuiEditScam extends GuiScreen{
	public GuiTextField displayName;
	public static List<String> list = new ArrayList<String>();
	
	public static String oldName;
	
	public GuiEditScam(String name) {
		oldName = name;
	}
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		if(button.id == 1) {
			ProfileUitls.editAccountScam(oldName, displayName.getText());
			this.mc.displayGuiScreen(new GuiTrustlist());
		}
		if(button.id == 3) {
			ProfileUitls.deleteAccountScam(oldName);
			this.mc.displayGuiScreen(new GuiTrustlist());
		}
